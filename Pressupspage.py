...
def get_pb():
    ...
    cur = con.cursor()
    pb_query = f"SELECT pressups_pb FROM bmi WHERE id={USER_ID}"
...
def update_pb():
    ...
    if pb == 0:
        con = sqlite3.connect('data.db')
        cur = con.cursor()
        pb_query = f'UPDATE bmi set pressups_pb = "{current_value}" WHERE id={USER_ID}'
        cur.execute(pb_query)
        con.commit()
