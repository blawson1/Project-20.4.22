...
def get_pb():
    ...
    cur = con.cursor()
    pb_query = f"SELECT plank_pb FROM bmi WHERE id={USER_ID}"
...
def update_pb():
    ...
    if pb == '00:00:00' and current_value != '00:00:00':
        con = sqlite3.connect('data.db')
        cur = con.cursor()
        pb_query = f'UPDATE bmi set plank_pb = "{current_value}" WHERE id={USER_ID}'
        cur.execute(pb_query)
        con.commit()
    ...
    # check current value is better than database
        ...
        # Booleans
        hours_more = current_hours > pb_hours
        minutes_more = current_hours == pb_hours and current_minutes > pb_minutes
        seconds_more = current_hours == pb_hours and current_minutes == pb_minutes and current_seconds > pb_seconds

        if hours_more or minutes_more or seconds_more:
            con = sqlite3.connect('data.db')
            cur = con.cursor()
            pb_query = f'UPDATE bmi set plank_pb = "{current_value}" WHERE id={USER_ID}'
            cur.execute(pb_query)
...
lbl_start_timer = tk.Label(master=frm_start_timer_label, text="Start stopwatch for plank", fg="green", width=30,
                           height=5)
...
lbl_reset_timer = tk.Label(master=frm_reset_timer_label, text="Start stopwatch for plank", fg="green", width=30, height=5)
